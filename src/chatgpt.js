...
const chatgpt = {
    ...
    selectors: {
        btns: {
            ...
            voice: 'button[data-start-dictation],'
                 + 'button[style*=composer-speech-button], button[data-testid*=composer-speech-button],'
        ...
        footer: 'div[data-testid=thread-disclaimer], div.wm-app-privacyPositioner',
    ...
    // * Spaces are inserted into button labels by parsing function names in camel/kebab/snake case
        ...
        closeSVGpath.setAttribute('fill', chatgpt.settings.scheme.isDark() ? 'white' : 'black')
    ...
    notify(...args) {
        ...
        if (thisQuadrantQueue.length > 1) {
            try { // to move old notifications
                for (const divID of thisQuadrantQueue.slice(0, -1)) { // exclude new div
                    const oldDiv = document.getElementById(divID) ; if (!oldDiv) continue
        ...
        // Show notification
        setTimeout(() => {
            notificationDiv.style.opacity = chatgpt.settings.scheme.isDark() ? 0.8 : 0.67 // show msg
    ...
    renderHTML(node) {
        ...
        for (const childNode of nodeContent)
            if (childNode.nodeType == Node.TEXT_NODE) {
                const text = childNode.nodeValue, matches = [...text.matchAll(reTags)]
