...
var functionAliases = [
  // whole function names to cross-alias
  [
    "activateAutoRefresh",
    "activateAutoRefresher",
    "activateRefresher",
    "activateSessionRefresher",
    "autoRefresh",
    "autoRefresher",
    "autoRefreshSession",
    "refresher",
...
var synonyms = [
  // constituent synonyms within function names
  ["activate", "turnOn"],
...
var targetTypes = [
  // for abstracted methods like get, insert
  "button",
  "link",
  "div",
  "response",
...
var chatgpt = {
  activateDarkMode: function () {
    if (chatgpt.isLightMode()) {
      ...
      try {
        ...
        console.error("🤖 chatgpt.js >> Headless menu not found");
        return;
      }
      setTimeout(() => {
        // open settings
        ...
        for (var menuItem of menuItems) {
          if (menuItem.text.match(/settings/i)) {
            menuItem.click();
            break;
          }
        }
        setTimeout(() => {
          ...
          try {
            ...
            console.error("🤖 chatgpt.js >> Theme selector not found");
            return;
          }
  ...
  fixHTMLTags: function (html) {
    ...
    html = html.replace(/ +/g, " "); // replace multiple spaces with a single space
    return html;
  },
  ...
  alert: function (title, msg, btns, checkbox, width) {
    ...
    title = this.fixHTMLTags(title);
    msg = this.fixHTMLTags(msg);
    ...
    modalMessage.innerText = msg ? msg : "";

    modalTitle = this.renderHTML(modalTitle); // render title
    modalMessage = this.renderHTML(modalMessage); // render message
    ...
    if (checkbox) {
      ...
      checkboxLabel.textContent =
        checkboxFn.name.charAt(0).toUpperCase() + // capitalize first char
        checkboxFn.name
    ...
    function keyHandler(event) {
      ...
      if (dismissKeys.includes(event.keyCode)) {
        for (var i = 0; i < alertProps.queue.length; i++) {
  ...
  autoRefresh: {
    ...
    deactivate: function () {
      if (this.isActive) {
        ...
        clearTimeout(this.isActive);
        this.isActive = null;
        console.info("↻ ChatGPT >> [" + chatgpt.autoRefresh.nowTimeStamp() + "] Auto refresh de-activated");
      } else {
        console.info("↻ ChatGPT >> [" + chatgpt.autoRefresh.nowTimeStamp() + "] Auto refresh already inactive!");
      }
    },
    ...
    toggle: {
      beacons: function () {
        if (chatgpt.autoRefresh.beaconID) {
          clearInterval(chatgpt.autoRefresh.beaconID);
          chatgpt.autoRefresh.beaconID = null;
  ...
  clearChats: function () {
    ...
    setTimeout(() => {
      ...
      var hasChats = false;
      for (var menuItem of menuItems) {
        if (menuItem.text.match(/clear conversations/i)) {
          menuItem.click();
          hasChats = true;
          break;
        }
      ...
      if (hasChats) {
        setTimeout(() => {
          for (var menuItem of menuItems) {
            if (menuItem.text.match(/confirm/i)) {
              menuItem.click();
              break;
            }
          }
  ...
  get: function (targetType, targetName = "") {
    ...
    if (!targetTypes.includes(targetType.toLowerCase())) {
      throw new Error("Invalid targetType: " + targetType + ". Valid values are: " + JSON.stringify(targetTypes));
    }

    // Validate targetName scoped to pre-validated targetType
    var targetNames = [],
      reTargetName = new RegExp("^get(.*)" + targetType + "$", "i");
    for (var prop in this) {
      if (typeof this[prop] === "function" && prop.match(reTargetName)) {
        targetNames.push(
          // add found targetName to valid array
    ...
    var targetFuncName = Object.keys(this).find(
      // find originally cased target function name
      function (name) {
        return name.toLowerCase() === targetFuncNameLower;
      }
    ); // test for match
  ...
  getChatInput: function () {
    return chatgpt.getChatBox().value;
  },
    ...
    if (/last|final/.test(strPos)) {
      ...
      var nthOfResponse =
        ...
        Number.isInteger(pos)
          ? pos // do nothing for integers
          : strPos.match(/^\d+/)
          ? strPos.match(/^\d+/)[0] // extract first digits for strings w/ them
          : // convert words to integers for digitless strings
            (/^(1|one|fir)(st)?$/.test(strPos)
              ? 1
              : /^(2|tw(o|en|el(ve|f))|seco)(nd|t[yi])?(e?th)?$/.test(strPos)
              ? 2
              : /^(3|th(ree|ir?))(rd|teen|t[yi])?(e?th)?$/.test(strPos)
              ? 3
              : /^(4|fou?r)(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 4
              : /^(5|fi(ve|f))(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 5
              : /^(6|six)(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 6
              : /^(7|seven)(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 7
              : /^(8|eight?)(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 8
              : /^(9|nine?)(teen|t[yi])?(e?th)?$/.test(strPos)
              ? 9
              : /^(10|ten)(th)?$/.test(strPos)
              ? 10
              : 1) *
  ...
  history: {
    ...
    activate: function () {
      this.isOff() ? this.toggle() : console.info("🤖 chatgpt.js >> Chat history is already enabled!");
    },
    deactivate: function () {
      this.isOn() ? this.toggle() : console.info("🤖 chatgpt.js >> Chat history is already disabled!");
    },
    toggle: function () {
      for (var navBtn of document.querySelectorAll('nav[aria-label="Chat history"] button')) {
        if (navBtn.textContent.match(/chat history/i)) navBtn.click();
        return;
      }
    },
  },
  ...
  isFullScreen: function () {
    ...
    return userAgentStr.includes("Chrome")
      ? window.matchMedia("(display-mode: fullscreen)").matches
      : userAgentStr.includes("Firefox")
      ? window.fullScreen
      : userAgentStr.match(/MSIE|rv:/)
  ...
  logout: function () {
    ...
    setTimeout(() => {
      var menuItems = document.querySelectorAll('a[role="menuitem"]') || [];
      for (var menuItem of menuItems) {
        if (menuItem.text.match(/log out/i)) {
          menuItem.click();
          break;
        }
      }
  ...
  notify: function (msg, position, notifDuration, shadow) {
    ...
    var vpYoffset = 23,
      vpXoffset = 27; // px offset from viewport border
    ...
    notificationDiv.quadrant =
      (notificationDiv.isTop ? "top" : "bottom") + (notificationDiv.isRight ? "Right" : "Left");
    ...
    var hideDelay = // set delay before fading
      fadeDuration > notifDuration
        ? 0 // don't delay if fade exceeds notification duration
        : notifDuration - fadeDuration; // otherwise delay for difference
    notificationDiv.hideTimer = setTimeout(function hideNotif() {
      // maintain notification visibility, then fade out
  ...
  printAllFunctions: function () {
    ...
    for (var functionName of functionNames) {
      console.info(
        "🤖 chatgpt.js >> " +
          (/chatgpt|other/.test(functionName[0]) ? "" : functionName[0] + ".") +
          functionName[1] +
          ": [" +
          ...
          !/chatgpt|other/.test(functionName[0]) // parent is chatgpt.obj
            ...
            : "Alias of") +
          ": " +
          (functionName[0] === "chatgpt"
            ? this[functionName[1]].name
            : functionName[0] !== "other"
  ...
  renderHTML: function (node) {
    ...
    if (!this.renderHTML.preWrapSet) {
      ...
      this.renderHTML.preWrapSet = true;
      setTimeout(() => {
        this.renderHTML.preWrapSet = false;
      }, 100);
    }
  ...
  sidebar: {
    isOn: function () {
      return !document.querySelector('button[aria-label*="Show sidebar"]');
    },
    isOff: function () {
      return !!document.querySelector('button[aria-label*="Show sidebar"]');
    },
    hide: function () {
      this.isOn() ? this.toggle() : console.info("🤖 chatgpt.js >> Sidebar already hidden!");
    },
    show: function () {
      this.isOff() ? this.toggle() : console.info("🤖 chatgpt.js >> Sidebar already shown!");
    },
  ...
  toggleScheme: function () {
    chatgpt.isLightMode() ? chatgpt.activateDarkMode() : chatgpt.activateLightMode();
  },
};
...
for (var buttonAction of buttonActions) {
  chatgpt[buttonAction + "Button"] = function handleButton(buttonIdentifier) {
    var button = buttonIdentifier.match(/^[.#]/)
      ...
      : /send/i.test(buttonIdentifier)
      ? document.querySelector('form button[class*="bottom"]')
      : /scroll/i.test(buttonIdentifier)
      ? document.querySelector('button[class*="cursor"]')
      : (function () {
          // get via text content
...
for (var prop in chatgpt) {
  ...
  do {
    // create new function per synonym per word per function
    ...
    for (var funcName in chatgpt) {
      if (typeof chatgpt[funcName] === "function") {
        ...
        for (var funcWord of funcWords) {
          var synonymValues = [].concat(
            ...synonyms // flatten into single array w/ word's synonyms
              .filter((arr) => arr.includes(funcWord.toLowerCase())) // filter in relevant synonym sub-arrays
              .map((arr) => arr.filter((synonym) => synonym !== funcWord.toLowerCase()))
          ); // filter out matching word
          for (var synonym of synonymValues) {
            // create function per synonym
            var newWords = [...funcWords]; // shallow copy funcWords
            newWords[newWords.indexOf(funcWord)] = synonym; // replace funcWord w/ synonym
            var newFuncName = newWords
              .map(
                (
                  newWord,
                  index // transform new words to create new name
            ...
            if (!chatgpt[newFuncName]) {
              // don't alias existing functions
...
} catch (error) {} // for Greasemonkey
try {
  module.exports = chatgpt;
} catch (error) {} // for CommonJS
