...
> [!WARNING]
> 
> **`activateDarkMode()` and `activateLightMode()` will be removed** in v4.12.1. Use `chatgpt.scheme.activateDark()` and `chatgpt.scheme.activateDark()` instead.
>
> **Top-level aliases will be removed** in v4.12.1 (e.g. `chatgpt.minify()`, `chatgpt.execute()`, `chatgpt.extractCode()`, `chatgpt.setScheme()` and `chatgpt.scheme.*`). Use the canonical APIs instead (e.g. `chatgpt.code.*`, `chatgpt.response.*` and `chatgpt.settings.scheme.*`)
